...
export function kanaToRomaji(value: string) {
  ...
  while (index < value.length) {
    ...
    const digraph = KANA_DIGRAPHS.find(([kana]) => kana === pair);
    if (digraph) {
      result += digraph[1];
      index += 2;
      continue;
    ...
    if (char === "っ" || char === "ッ") {
      ...
      const nextDigraph = KANA_DIGRAPHS.find(([kana]) => kana === nextPair);
      const nextRomaji =
        nextDigraph?.[1] || KANA_MONOGRAPHS[value[index + 1] ?? ""] || "";
      result += nextRomaji[0] ?? "";
      index += 1;
      continue;
    }
    ...
    result += KANA_MONOGRAPHS[char] ?? char;
    index += 1;
  }
...
function tokenizeRomaji(value: string) {
  ...
  const cleaned = source.toLowerCase().replace(/[^a-z\s/,-]/g, " ").trim();
  const tokens: string[] = [];
  let index = 0;
